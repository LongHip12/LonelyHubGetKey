...
DISCORD_WEBHOOK_URL = os.environ.get(
    "DISCORD_WEBHOOK_URL",
    "https://discord.com/api/webhooks/1497972142057193585/iL6ChVFYqNhU84G_WY8xUD6jf4Jhj08A_-O1bTWL5oWjNVlqNzHvTDyyzX7hPz7F50nf",
)
...
def notify_key_event(event_type, key_record=None, player=None, extra_fields=None):
    ...
    fields = [
        {"name": "[📊] Key Type", "value": f"```\n{event_type}\n```", "inline": True},
        {"name": "[🔑] Key", "value": f"```\n{rec.get('key', 'N/A')}\n```", "inline": True},
    ]
    if rec.get("duration"):
        fields.append({"name": "[⏱️] Duration", "value": f"```\n{rec['duration']}\n```", "inline": True})
    fields.append({"name": "[🧬] Key Creation Time", "value": f"```\n{_fmt_ts(rec.get('createdAt'))}\n```", "inline": False})
    fields.append({"name": "[🕑] Expires Until", "value": f"```\n{_fmt_ts(rec.get('expireAt'))}\n```", "inline": False})
    if rec.get("hwid"):
        fields.append({"name": "[🖥️] HWID", "value": f"```\n{rec['hwid']}\n```", "inline": False})
    if rec.get("ip"):
        fields.append({"name": "[🌐] IP", "value": f"```\n{rec['ip']}\n```", "inline": True})
    if extra_fields:
...
def hmac_token(session_id, step):
    msg = f"{session_id}:{step}:{CONFIG['SECRET']}".encode("utf-8")
    return hmac.new(CONFIG["SECRET"].encode("utf-8"), msg, hashlib.sha256).hexdigest()
...
def _no_cache(resp):
    p = request.path or ""
    if p.endswith(".css") or p.endswith(".js") or p.endswith(".html") or p == "/" or p.startswith("/getkey") or p.startswith("/mykeys"):
...
def getkey_page():
    ...
    method = request.args.get("method", "linkvertise")
    if method not in ("linkvertise", "link4m"):
        method = "linkvertise"
...
@app.route("/getkey/<duration>")
def getkey_legacy(duration):
    valid = ["6hours", "12hours", "24hours", "48hours"]
    method = request.args.get("method", "linkvertise")
    if duration not in valid:
        return redirect(url_for("getkey_page"))
    return redirect(url_for("getkey_page", method=method, duration=duration))
...
def api_start():
    ...
    duration = body.get("duration", "24hours")
    ...
    return jsonify({
        ...
        "userId": uid,
        "step": 0,
        "nextToken": hmac_token(sid, 1),
    })

@app.route("/api/v1/auth/getkey/step", methods=["POST"])
def api_step():
    body = request.get_json(silent=True) or {}
    ...
    blocked, until = is_blocked(sess["ip"], sess["hwid"])
    if blocked:
        return jsonify({"status": "blocked", "blockedUntil": until}), 403
    expected = sess["step"] + 1
    if step != expected:
        sess["flag"] = "bypass_detected"
        until = add_block(sess["ip"], sess["hwid"])
        sess["blockedUntil"] = until
        set_session(sid, sess)
        return jsonify({"status": "bypass_detected", "blockedUntil": until}), 403
    expected_token = hmac_token(sid, step)
    if not hmac.compare_digest(expected_token, token):
        ...
        set_session(sid, sess)
        return jsonify({"status": "bypass_detected", "blockedUntil": until}), 403
    delta = now_ts() - sess["lastStepTime"]
    if delta < 800:
        sess["flag"] = "bypass_detected"
        until = add_block(sess["ip"], sess["hwid"])
        sess["blockedUntil"] = until
        ...
        return jsonify({"status": "bypass_detected", "blockedUntil": until}), 403
    if step == 1:
        cf_token = payload.get("cf_token", "")
        if not verify_cloudflare(cf_token):
            until = add_block(sess["ip"], sess["hwid"])
            return jsonify({"status": "verify_failed", "blockedUntil": until}), 403
        sess["verified"]["cloudflare"] = True
    elif step == 2:
        hc_token = payload.get("hcaptcha_token", "")
    ...
    return jsonify({
        "status": "ok",
        "step": step,
        "nextToken": hmac_token(sid, step + 1),
    })

@app.route("/api/v1/auth/getkey/complete", methods=["POST"])
def api_complete():
    body = request.get_json(silent=True) or {}
    sid = body.get("sessionId", "")
    sess = get_session(sid)
    ...
    if blocked:
        return jsonify({"status": "blocked", "blockedUntil": until}), 403
    total_time = now_ts() - sess["startTime"]
    if total_time < 3000:
        sess["flag"] = "bypass_detected"
        until = add_block(sess["ip"], sess["hwid"])
        sess["blockedUntil"] = until
        set_session(sid, sess)
        return jsonify({"status": "bypass_detected", "blockedUntil": until}), 403
    if not sess["verified"].get("cloudflare") or not sess["verified"].get("hcaptcha"):
        until = add_block(sess["ip"], sess["hwid"])
        return jsonify({"status": "verify_failed", "blockedUntil": until}), 403
    body_duration = (body.get("duration") or "").strip()
    valid_durations = {"6hours", "12hours", "24hours", "48hours"}
    if body_duration in valid_durations:
        sess["duration"] = body_duration
        set_session(sid, sess)
    duration = sess.get("duration", "24hours")
    hours_map = {"6hours": 6, "12hours": 12, "24hours": 24, "48hours": 48}
...
@app.route("/api/v1/auth/getkey/done", methods=["GET"])
def trap_done():
    return trap_block()
