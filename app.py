...
def step_page(sid, step):
    ...
    if total <= 1:
        return redirect(url_for("finish_page", sid=sid))
    if step != sess.get("step", 0) + 1 or step > total - 1:
        return redirect(url_for("getkey_page"))
    return render_template(
        "step.html",
        sid=sid,
...
def api_start():
    ...
    set_session(sid, sess)
    if total_steps <= 1:
        target = f"{base_url()}/getkey/finish/{sid}"
    else:
        target = f"{base_url()}/getkey/step/{sid}/1"
...
def api_continue():
    ...
    if step != expected or step > total - 1:
    ...
    if step == total - 1:
        ...
        target = f"{base_url()}/getkey/step/{sid}/{step + 1}"
